(() => {
  ...
  let interventionFocusToken=0;
  function focusInterventionOnMap(a,zoom=17){
    const lat=Number(a?.lat),lng=Number(a?.lng);
    if(!Number.isFinite(lat)||!Number.isFinite(lng))return;
    const token=++interventionFocusToken;
    const apply=()=>{
      if(token!==interventionFocusToken||state.selected!==a)return;
      map.invalidateSize();
      map.setView([lat,lng],zoom);
    };
    // Google Maps peut finir son affichage après l'ouverture de la page Carte.
    // Ces rappels empêchent la vue générale de remplacer le zoom d'intervention.
    apply();
    [80,280,700].forEach(delay=>setTimeout(apply,delay));
    Promise.resolve(map.ready).then(()=>setTimeout(apply,40)).catch(()=>{});
  }
  function selectAddress(a,remember=true){state.selected=a;$("addressSearch").value=a.adresse;$("addressSearchFull").value=a.adresse;$("results").innerHTML=$("resultsFull").innerHTML="";$("interventionCard").classList.remove("hidden");$("selectedAddress").textContent=a.adresse;interventionLayer.clearLayers();resourceLayer.clearLayers();L.marker([a.lat,a.lng],{icon:interventionIcon()}).bindPopup(`<strong>🔥 Intervention</strong><br>${esc(a.adresse)}`).addTo(interventionLayer).openPopup();renderNearest();if(remember)saveHistory(a);showView("map");focusInterventionOnMap(a,17);window.dispatchEvent(new CustomEvent("firemap:intervention-start",{detail:{...a,lat:Number(a.lat),lng:Number(a.lng)}}))}
  function clearIntervention(){const previous=state.selected;interventionFocusToken++;state.selected=null;state.nearest=[];interventionLayer.clearLayers();resourceLayer.clearLayers();applyHydrantCallVisibility(true);$("interventionCard").classList.add("hidden");$("addressSearch").value=$("addressSearchFull").value="";updateAddressCounts();map.setView(CENTER,14);window.dispatchEvent(new CustomEvent("firemap:intervention-end",{detail:previous||null}))}
  ...
  window.fireMapInternal={map,state,showView,openHydrantForm:openForm,navUrl,toast,esc,norm,addressNorm,getAddresses:()=>state.addresses,getHydrants:()=>state.hydrants,selectAddress,clearIntervention,nearestAddress,rankHydrantsByRoad,drawRoadRoutes,isFireRelatedCall,applyHydrantCallVisibility,focusInterventionOnMap};
